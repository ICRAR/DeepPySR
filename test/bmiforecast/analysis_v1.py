"""
Simplified analysis for bmiforecast results.
...
numbers (see test_bmiforecast.py). No re-evaluation of any formula happens here.
...
BMI comparison rows (age-specific / longitudinal) are read directly from
bmi/analysis_v1/bmi_best_models_metrics.csv -- no re-computation. KANSym is
excluded everywhere (degenerate: r2 clipped from negative at every age).

bmiforecast has no model of its own at age 8 (forecasting starts at age 10) --
age 8 is filled in from bmi's own age-specific age-8 results, since that IS the
age-8 model. bmiforecast's "age 23" wave and bmi's own "age 23" wave are the
same raw source as each other (unaffected here); AGE_RELABEL exists so that,
when this file's output is later joined against newbmiforecast's natively-labeled
"age 22" wave (same underlying measurement, different source naming), the ages
line up -- see newbmiforecast/analysis.py.
...
from bmiforecast_utils import YEARS, actual_age, _is_bmi_col
...
BMI_AGE8_DIR = os.path.join(current_dir, '..', 'bmi', 'results_bmi_all', 'age_specific', 'age_8')

FORECAST_YEARS = [y for y in YEARS if y != 8]           # [10, 13, 16, 20, 23, 26]
FORECAST_AGES = [actual_age(y) for y in FORECAST_YEARS]  # [10, 14, 17, 20, 23, 27]

INTERP_COMPLEXITY_THRESHOLD = 30
# bmiforecast/bmi's "age 23" wave is the same underlying measurement as
# newbmiforecast's native "age 22" wave -- different source naming for the
# same thing, not a real 1-year gap.
AGE_RELABEL = {23: 22}
...
def _get_feature_cols_for_year(rolling_df, year, model_type='deeppysr'):
    ...
    prior_years = FORECAST_YEARS[:FORECAST_YEARS.index(year)] if year in FORECAST_YEARS else []
    prior_pred_cols = []
...
def load_age_metrics(age_dir, rolling_df, year):
    ...
    def _winning_config(family_dir, max_complexity=None):
        """(config_name, metrics, formula_row) of the config with highest cv
        r2 among those with a qualifying formula (all configs if
...
def load_bmiforecast_results(rolling_df):
    ...
    for year, age in zip(FORECAST_YEARS, FORECAST_AGES):
        ...
        if not os.path.isdir(age_dir):
            continue
        display_age = AGE_RELABEL.get(age, age)
        for display, m in load_age_metrics(age_dir, rolling_df, year).items():
            rows.append({'age': display_age, 'display_model': display, 'source': 'BMI Forecast', **m})
...
def fill_age8_from_bmi(df_forecast, df_age_specific):
    """bmiforecast has no model of its own at age 8 (forecasting starts at
    age 10) -- borrow bmi's own age-8 age-specific results, since that IS the
    age-8 model."""
    age8 = df_age_specific[df_age_specific['age'] == 8].copy()
    age8['source'] = 'BMI Forecast'
    return pd.concat([df_forecast, age8], ignore_index=True)
...
FORECAST_AGES_DISPLAY = sorted({AGE_RELABEL.get(a, a) for a in [8] + FORECAST_AGES})


def plot_comparison_vs_age(df_forecast, df_age_specific, df_longitudinal, out_dir):
    """3×5 grid: rows = source, cols = R2 / RMSE / MAE / Pearson r / Complexity."""
    sources = [
...
def plot_deeppysr_only(df_forecast, df_age_specific, df_longitudinal, out_dir):
    """2×5: DeepPySR (row 0) and DeepPySR Interpretable (row 1) across 3 sources.
...
_CDC_BMI_EXT_CACHE = {}
_CDC_BMI_EXT_PATH = os.path.join(current_dir, '..', 'newbmiforecast', 'bmizscore', 'bmi-age-2022.csv')
...
_WHO_LMS_CACHE = {}
_WHO_LMS_DIR = os.path.join(current_dir, '..', 'newbmiforecast', 'bmizscore')
...
def _cdc_zscore_to_bmi(z, agemos, sex):
    """Raw BMI from a CDC-extended BMI-for-age z-score at a given age in months."""
    df = _load_cdc_bmi_extended(sex)
    if df is None or pd.isna(z):
        return np.nan
    idx = (df['agemos'] - agemos).abs().idxmin()
    row = df.loc[idx]
    return _lms_zscore_to_bmi(float(row['L']), float(row['M']), float(row['S']), z)


def _early_bmi_observations(base_idx, cid, sex):
    """Birth / year-1 / year-5 raw BMI, inverted from the cohort's precomputed
    BMI-for-age z-scores (birth_bmifa, y1_bmifa, y5_bmiz) via the same WHO
    (0-2y) / CDC-extended (2y+) LMS tables used for the growth-chart
    background -- raw weight/height for these ages aren't retained in
    base_dataset.csv, only the z-scores. Ages assumed nominal (0, 12, 60
    months) since exact ages at these early visits aren't available.
    Returns {age_years: bmi}.
    """
    out = {}
    if cid not in base_idx.index:
        return out
    row = base_idx.loc[cid]
    if 'birth_bmifa' in row.index and pd.notna(row['birth_bmifa']):
        v = _who_zscore_to_bmi(float(row['birth_bmifa']), 0, sex)
        if pd.notna(v):
            out[0] = v
    if 'y1_bmifa' in row.index and pd.notna(row['y1_bmifa']):
        v = _who_zscore_to_bmi(float(row['y1_bmifa']), 12, sex)
        if pd.notna(v):
            out[1] = v
    if 'y5_bmiz' in row.index and pd.notna(row['y5_bmiz']):
        v = _cdc_zscore_to_bmi(float(row['y5_bmiz']), 60, sex)
        if pd.notna(v):
            out[5] = v
    return out
...
_BMI_AGE8_FITTED_DIR = os.path.join(BMI_AGE8_DIR, 'baselines', '_fitted_models')
_BMI_AGE8_FEATURE_LIST_PATH = os.path.join(BMI_AGE8_DIR, 'baselines', 'ElasticNet', 'feature_importance.csv')


def _load_bmi_age8_feature_cols():
    """Ordered feature list bmi's age-8 baseline models were trained on
    (recovered from a baseline's own feature_importance.csv, which lists
    features in training column order). 'age' is dropped: feature_importance.csv
    lists it, but the fitted models themselves expect one fewer column
    (124, not 125) -- confirmed via each model's own n_features_in_ -- since
    it's constant within a single age-specific fit and was excluded from X
    before training."""
    if not os.path.exists(_BMI_AGE8_FEATURE_LIST_PATH):
        return None
    cols = pd.read_csv(_BMI_AGE8_FEATURE_LIST_PATH)['feature'].tolist()
    return [c for c in cols if c != 'age']


def _bmi_age8_baseline_predictions(rolling_df, sampled_ids):
    """Age-8 baseline predictions using bmi's own fitted joblib models
    (bmi/results_bmi_all/age_specific/age_8/baselines/_fitted_models), applied
    directly to bmiforecast's rolling_dataset rows. bmi's baseline features
    already use the same SUM_PGS* naming as rolling_dataset (only the
    DeepPySR/PySR formulas went through a display-only PGS rename -- see
    _bmi_age8_formula_predictions).
    """
    import joblib as _joblib
    preds = {}
    feat_cols = _load_bmi_age8_feature_cols()
    if feat_cols is None:
        return preds
    rolling_sub = rolling_df[rolling_df['child_id'].isin(sampled_ids)].set_index('child_id')
    rolling_sub = rolling_sub.reindex(sampled_ids)
    missing = [c for c in feat_cols if c not in rolling_sub.columns]
    if missing:
        return preds

    X = rolling_sub[feat_cols]

    for name in ['ElasticNet', 'ExtraTrees', 'MLP', 'RandomForest', 'XGBoost']:
        path = os.path.join(_BMI_AGE8_FITTED_DIR, f'{name}.joblib')
        if not os.path.exists(path):
            continue
        try:
            ...
            continue
        preds[name] = dict(zip(sampled_ids, yp))
    return preds


def _map_bmi_pgs_formula(formula_str):
    """bmi's own SR pipeline (bmi_utils.load_bmi_agg_data) renames
    SUM_PGS* -> PGS* purely for display before fitting DeepPySR/PySR, so
    formula strings from bmi_best_models_metrics.csv use the bare PGS* name.
    Reverse it so the formula can be evaluated against rolling_dataset's own
    SUM_PGS* columns."""
    import re as _re
    return _re.sub(r'(?<!SUM_)\bPGS(\d+)\b', r'SUM_PGS\1', str(formula_str))


def _bmi_age8_formula_predictions(rolling_df, sampled_ids):
    """Age-8 DeepPySR/DeepPySR (Interpretable)/PySR predictions using the
    winning (highest-r2) formula from bmi_best_models_metrics.csv's own
    age-specific age-8 rows -- bmiforecast has no age-8 model of its own.
    """
    preds = {}
    if not os.path.exists(BMI_BEST_MODELS_CSV):
        return preds
    df = pd.read_csv(BMI_BEST_MODELS_CSV)
    sub = df[(df['age'] == 8) & (df['type'] == 'age-specific')]
    rolling_sub = rolling_df[rolling_df['child_id'].isin(sampled_ids)].set_index('child_id')
    rolling_sub = rolling_sub.reindex(sampled_ids)

    for model_label, display, model_type in [
            ('Best DeepPySR', 'DeepPySR', 'deeppysr'),
            ('Interpretable DeepPySR', 'DeepPySR (Interpretable)', 'deeppysr'),
            ('PySR', 'PySR', 'pysr')]:
        row = sub[sub['display_model'] == model_label]
        if row.empty or pd.isna(row.iloc[0].get('formula')):
            continue
        formula = _map_bmi_pgs_formula(row.iloc[0]['formula'])
        try:
            yp = evaluate_formula(formula, rolling_sub, model_type=model_type)
        except Exception:
            continue
        preds[display] = dict(zip(sampled_ids, yp))
    return preds


def _bmi_age8_model_predictions(rolling_df, sampled_ids):
    """Age-8 predictions for all 7 models, merging bmi's own genuine
    out-of-fold CV predictions (for participants who provided an age-8
    observation, i.e. contributed to fitting bmi's age-8 baseline/no-CV
    models -- using them there would be leakage) with bmi's full-data
    (no-CV) baseline joblib models / bmi_best_models_metrics.csv formulas
    (for participants with no age-8 observation, outside bmi's training set
    entirely, so no leakage concern).
    """
    nocv_preds = {**_bmi_age8_baseline_predictions(rolling_df, sampled_ids),
                  **_bmi_age8_formula_predictions(rolling_df, sampled_ids)}

    oof_preds = {}
    for name in ['ElasticNet', 'ExtraTrees', 'MLP', 'RandomForest', 'XGBoost']:
        oof_preds[name] = _oof_predictions_from_dir(
            os.path.join(BMI_AGE8_DIR, 'baselines', name), is_grid_family=False)
    oof_preds['DeepPySR'] = _oof_predictions_from_dir(
        os.path.join(BMI_AGE8_DIR, 'deeppysr'), is_grid_family=True)
    oof_preds['DeepPySR (Interpretable)'] = _oof_predictions_from_dir(
        os.path.join(BMI_AGE8_DIR, 'deeppysr'), is_grid_family=True,
        max_complexity=INTERP_COMPLEXITY_THRESHOLD, formula_prefix='relationships_fold*.csv')
    oof_preds['PySR'] = _oof_predictions_from_dir(
        os.path.join(BMI_AGE8_DIR, 'pysr'), is_grid_family=True)
...
def _all_year_preds(rolling_df, sampled_ids):
    ...
    YEARS for the given participant ids -- see _year_model_predictions
    (ages 10-27) and _bmi_age8_model_predictions (age 8, bmi's own pipeline)
    for the genuine, leak-free-by-construction sourcing rule.
    """
    ages = [actual_age(y) for y in YEARS]
    year_preds = {8: _bmi_age8_model_predictions(rolling_df, sampled_ids)}
    for year, age in zip(YEARS, ages):
        if year not in FORECAST_YEARS:
            continue
        # NB: age_dir must use the real (unrelabeled) age -- results are
        # stored under age_23, never age_22; AGE_RELABEL only renames that
        # wave to 22 for display/x-axis purposes when comparing against
        # newbmiforecast's natively-labeled age-22 wave.
        age_dir = os.path.join(FORECAST_RESULTS_DIR, f'age_{age}')
...
def _select_deeppysr_advantage_ids(rolling_df, base_df, n_obs_filter, n_sample):
    ...
    ages = [actual_age(y) for y in YEARS]
    display_ages = [AGE_RELABEL.get(a, a) for a in ages]
...
def _render_trajectory_grid(sampled_ids, year_preds, base_idx, out_dir, out_filename, suptitle):
    ...
    for p_num, (ax, cid) in enumerate(zip(axes, sampled_ids), start=1):
        ax.set_box_aspect(1)
        sex = base_idx.loc[cid, 'sex'] if (cid in base_idx.index and 'sex' in base_idx.columns) else np.nan
        sex = int(sex) if pd.notna(sex) and int(sex) in (1, 2) else 1

        model_series = {}
        for model in MODEL_TYPES:
            xs, ys = [], []
            for year, age in zip(YEARS, display_ages):
        ...
        obs_x, obs_y = [], []
        for y, age in zip(YEARS, display_ages):
            col = f'y{y}bmi'
        ...
        ax.set_ylabel('BMI')
        ax.set_xticks(sorted(set(display_ages) | set(early.keys())))
...
def plot_sparse_trajectories(rolling_df, base_df, out_dir, n_obs_filter=2, n_sample=12, seed=42):
    ...
    background is most informative). See _all_year_preds for the prediction
    sourcing rule (genuine out-of-fold CV where the participant has an
    observation at that age, full no-CV model otherwise).
    """
    base_idx = base_df.set_index('child_id')
    ...
    _render_trajectory_grid(
        ...
        out_filename=f'bmiforecast_trajectories_{n_obs_filter}_obs.png',
...
def plot_deeppysr_best_trajectories(rolling_df, base_df, out_dir, n_obs_filter=2, n_sample=12):
    ...
    _render_trajectory_grid(
        sampled_ids, year_preds, base_idx, out_dir,
        out_filename=f'bmiforecast_trajectories_deeppysr_best_{n_obs_filter}_obs.png',
        suptitle=f'BMI Trajectories — Participants where DeepPySR Outperforms Other Models ({n_obs_filter} Observed Values)')


# ── Formula demonstration (post-hoc, per selected participant) ─────────────────
...
def save_formula_demonstration(rolling_df, base_df, df_forecast, out_dir, n_obs_filter=2, n_sample=10):
    ...
    Scope: forecast ages only (10-27); age 8's formula comes from bmi's own
    age-specific model, which uses a different feature-naming convention
    (e.g. PGS002313 vs rolling_dataset's SUM_PGS002313) not reconciled here.
    ...
    for p_num, cid in enumerate(sampled_ids, start=1):
        ...
        for year, age in zip(FORECAST_YEARS, FORECAST_AGES):
            ...
            for label, model_type in DEMO_MODELS:
                ...
                for vname, (val, vtype, summary, pct, p_value) in sorted(var_info.items()):
                    csv_rows.append({
                        'n_obs': n_obs_filter, 'participant_number': p_num, 'child_id': cid,
                        'age': display_age, 'model': label, 'formula': formula,
...
if __name__ == '__main__':
    ...
    df_forecast = load_bmiforecast_results(rolling_df)
    ...
    print('=== Filling age 8 from bmi age-specific results ===')
    df_forecast = fill_age8_from_bmi(df_forecast, df_age_specific)
