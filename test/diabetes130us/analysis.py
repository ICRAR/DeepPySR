...
def process_results():
    all_data = []
    base_dir = os.path.join(current_dir, "../../archive/results/results_diab130_all")
...
def save_results(df):
    ...
    plot_csv_path = os.path.join(current_dir, 'diab130_best_models_metrics.csv')
...
def plot_best_models():
    ...
    """
    df = pd.read_csv(os.path.join(current_dir, 'diab130_best_models_metrics.csv'))
    ...
    for j, metric in enumerate(metrics):
        ...
        if plot_df.empty:
            ...
            ax.set_title(f'Diabetes130 - {metric.upper()}')
