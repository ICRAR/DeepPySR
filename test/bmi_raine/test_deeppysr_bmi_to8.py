...
_LOAD_FN = {
    'PGS':    (load_data_PGS_only,  'results_bmi/results_bmi_PGS'),
    'to8':    (load_data_keepto8,   'results_bmi/results_bmi_to8'),
    'PGSto8': (load_data_PGSto8,    'results_bmi/results_bmi_PGSto8'),
}
...
def main():
    ...
    run_name = f"age_{args.age}_{TARGET}"
    print(f"\n--- Run: {run_name} ---")
    ...
    # DeepPySR models — trained on the full feature set.
    ...
    for cfg_name, cfg_overrides in deeppysr_configs.items():
        ...
        deeppysr_out = os.path.join(run_out, "deeppysr", full_name, "all_features")
