...
def _data():
    """Training used StratifiedKFold(n_splits=5, shuffle=True, random_state=42) on
    y itself (cv_kwargs: stratify_by=y) -- task='classification' alone already
    selects that same split in analysis_v1_utils, so no explicit stratify= needed."""
    X, y = load_heart_cleveland_data(binary=True)
    return X, y


def process_results():
    """Best DeepPySR/PySR/KAN/KANSym metrics come from predictions.csv (genuine
    held-out, pooled). Interpretable DeepPySR uses per-fold held-out formula
    evaluation (no predictions.csv exists for that constrained operating point)."""
    X, y = _data()
    # heart's classification baselines are LogisticRegression/ExtraTrees/MLP/
    # RandomForest/XGBoost (no ElasticNet, which is regression-only) --
    # leak_free_process_and_select's default baseline_models tuple assumes
    # regression baselines, so it must be overridden here or LogisticRegression
    # (the actual best baseline, f1=0.821) silently drops out of best_df.
    all_df, best_df = leak_free_process_and_select(
        BASE_DIR, X, y, task=TASK, interp_max_complexity=INTERP_MAX_COMPLEXITY,
        baseline_models=('LogisticRegression', 'ExtraTrees', 'MLP', 'RandomForest', 'XGBoost'))
...
def compute_se_and_wilcoxon(best_df):
    ...
    metrics_csv_path = os.path.join(current_dir, 'heart_best_models_metrics.csv')
...
def plot_vps_vpr_ablation(all_df):
    """Ablation: VPS/VPR effect -- best F1 per config at the model's own
    default (r2w=1, lambda=0.001) operating point, genuinely held-out per fold."""
    ...
    for j, (metric, mlabel) in enumerate(zip(metrics, metric_labels)):
        ...
        ax.set_title(f'Heart Disease – {mlabel}', fontsize=13, fontweight='bold')
...
def plot_pareto_ablation(all_df):
    ...
    Each grid point scored genuinely per-fold via pareto_point= filtering."""
    X, y = _data()
    metrics = ['accuracy', 'f1', 'auc', 'complexity']
    metric_labels = ['Accuracy', 'F1', 'AUC', 'Complexity']
...
def _load_hof_data(model_dir):
    ...
    dynamics, not a held-out claim, so unaffected by the leak."""
...
def plot_pareto_front_rmse(all_df):
    ...
    ax.set_title('Heart Disease – Pareto Front: Complexity vs Training RMSE', fontsize=14, fontweight='bold')
