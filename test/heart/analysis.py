...
def plot_pareto_front_rmse(df):
    ...
    if not deep_df.empty:
        ...
        model_dir = os.path.join(current_dir, 'results_heart_all', 'deeppysr', base_model)
    ...
    if not pysr_df.empty:
        ...
        pysr_model_dir = os.path.join(current_dir, 'results_heart_all', 'pysr', best_pysr_name)
    ...
    ax.set_title('Heart Disease – Pareto Front: Complexity vs Error', fontsize=14, fontweight='bold')
