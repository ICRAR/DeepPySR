...
def plot_vps_vpr_ablation(df):
    ...
    csv_df.to_csv(os.path.join(current_dir, 'results_heart_all', 'ablation_vps_vpr.csv'), index=False)
    print(f"VPS/VPR ablation data saved to {os.path.join(current_dir, 'results_heart_all', 'ablation_vps_vpr.csv')}")
    ...
    plt.tight_layout()
    out = os.path.join(current_dir, 'results_heart_all', 'ablation_vps_vpr.png')
...
def plot_pareto_ablation(df):
    ...
    csv_df = pd.concat([deep_df[['label'] + metrics], pysr_sub[['label'] + metrics]], ignore_index=True)
    csv_df.to_csv(os.path.join(current_dir, 'results_heart_all', 'ablation_pareto.csv'), index=False)
    print(f"Pareto ablation data saved to {os.path.join(current_dir, 'results_heart_all', 'ablation_pareto.csv')}")
    ...
    plt.tight_layout()
    out = os.path.join(current_dir, 'results_heart_all', 'ablation_pareto.png')
    plt.savefig(out, dpi=300, bbox_inches='tight')
    plt.close()
...
def plot_pareto_front_rmse(df):
    """Scatter plot of complexity vs (1-F1) showing the Pareto front for DeepPySR and PySR variants."""
    results_dir = os.path.join(current_dir, 'results_heart_all')
    ...
    for d in [deep_df, pysr_df]:
        d['error'] = 1.0 - d['f1'].fillna(0)

    deep_df = deep_df[deep_df['complexity'].notna()]
    pysr_df = pysr_df[pysr_df['complexity'].notna()]

    if deep_df.empty and pysr_df.empty:
        print("No data for pareto front RMSE plot")
        return

    fig, ax = plt.subplots(figsize=(10, 7))

    if not deep_df.empty:
        pf = _pareto_front_steps(deep_df['complexity'].tolist(), deep_df['error'].tolist())
        if pf:
            px, py = zip(*pf)
    ...
    if not pysr_df.empty:
        pf_pysr = _pareto_front_steps(pysr_df['complexity'].tolist(), pysr_df['error'].tolist())
        if pf_pysr:
            px, py = zip(*pf_pysr)
    ...
    ax.set_ylabel('1 – F1 (error)', fontsize=13)
    ax.set_title('Heart Disease – Pareto Front: Complexity vs (1-F1)', fontsize=14, fontweight='bold')
