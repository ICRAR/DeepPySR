...
def main():
    output_dir = os.path.join(current_dir, 'results_student_stats')
    if not os.path.exists(output_dir):
        os.makedirs(output_dir, exist_ok=True)

    deep_results_dir = os.path.join(current_dir, 'deep_analysis_results')
    ...
    for subject in ['mat', 'por']:
        ...
        rel_file = os.path.join(deep_results_dir, subject, 'relationships.csv')

        if not os.path.exists(rel_file):
            print(f"Relationships file not found for {subject}: {rel_file}")
            continue
        ...
        if not layer1.empty:
            ...
            subject_label = 'Math' if subject == 'mat' else 'Portuguese'
            output_file = os.path.join(output_dir, f'stats_{subject}.txt')
            run_stats(X, y, features, f"Student Performance ({subject_label})", formula=formula, metrics=metrics, output_file=output_file)
        else:
            print(f"No layer 1 formula found for {subject}")
