...
def run_stats(X, y, features, title, formula=None, metrics=None, output_file=None):
    ...
    try:
        # Logistic regression for classification if target is binary
        if len(y.unique()) == 2:
            model = sm.Logit(y, X_subset).fit()
        else:
            model = sm.MNLogit(y, X_subset).fit()
...
def main():
    output_dir = os.path.join(current_dir, 'results_diabetes_stats')
    if not os.path.exists(output_dir):
        os.makedirs(output_dir, exist_ok=True)
        
    deep_results_dir = os.path.join(current_dir, 'results_diabetes_deep')
    ...
    if not layer1.empty:
        ...
        output_file = os.path.join(output_dir, 'stats_diabetes.txt')
        run_stats(X, y, features, "Diabetes", formula=formula, metrics=metrics, output_file=output_file)
    else:
