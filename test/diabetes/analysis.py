...
def _stratify():
    """Diabetes training used StratifiedKFold on y itself (cv_kwargs: stratify_by=y);
    task='classification' alone already selects that split in analysis_v1_utils, so no
    explicit stratify is needed (confirmed by test/diabetes/analysis_v1/run_analysis.py)."""
    X, y = load_diabetes_brfss_data()
    return X, y, None


def process_results():
    X, y, stratify = _stratify()
...
def compute_se_and_wilcoxon(best_df):
    ...
    for _, row in best_df.iterrows():
        fd = _fold_data_for_row(row, X, y, stratify)
        if fd is not None:
...
def plot_best_models():
    ...
    for j, metric in enumerate(metrics):
        ...
        if plot_df.empty:
            ...
            ax.set_title(f'Diabetes - {metric.upper()}')
...
def plot_vps_vpr_ablation(all_df):
    ...
    model's own default (r2w=1, lambda=0.001) operating point, genuinely held-out per fold."""
...
def plot_pareto_ablation(all_df):
    ...
    Each grid point scored genuinely (get_best_formula_from_raw pareto_point=)."""
    X, y, stratify = _stratify()
    metrics = ['accuracy', 'f1', 'auc', 'complexity']
    metric_labels = ['Accuracy', 'F1', 'AUC', 'Complexity']
        ...
        for r2w in R2W_LIST:
            for lam in LAMBDA_LIST:
                formula, complexity, m = get_best_formula_from_raw(
                    v_path, X, y, task=TASK, model_type='deeppysr', stratify=stratify,
...
def _load_hof_data(model_dir):
    """SR search's own training-time complexity/loss log -- optimizer dynamics,
    not a held-out generalization claim, so unaffected by the formula-selection leak."""
...
def plot_pareto_front_rmse(all_df):
    ...
    if not hof_data.empty:
        ...
        if pf:
            ...
            ax.scatter(px, py, c='#2166ac', s=100, zorder=5, marker='D', label='DeepPySR (train loss)')
    ...
    if not hof_pysr.empty:
        ...
        if pf_pysr:
            ...
            ax.scatter(px, py, c='#cc4400', s=100, zorder=5, marker='D', label='PySR (train loss)')
