...
def plot_vps_vpr_ablation(df):
    ...
    csv_df.to_csv(os.path.join(current_dir, 'results_stroke_all', 'ablation_vps_vpr.csv'), index=False)
    print(f"VPS/VPR ablation data saved to {os.path.join(current_dir, 'results_stroke_all', 'ablation_vps_vpr.csv')}")
    ...
    plt.tight_layout()
    out = os.path.join(current_dir, 'results_stroke_all', 'ablation_vps_vpr.png')
...
def plot_pareto_ablation(df):
    ...
    csv_df = pd.concat([deep_df[['label'] + metrics], pysr_sub[['label'] + metrics]], ignore_index=True)
    csv_df.to_csv(os.path.join(current_dir, 'results_stroke_all', 'ablation_pareto.csv'), index=False)
    print(f"Pareto ablation data saved to {os.path.join(current_dir, 'results_stroke_all', 'ablation_pareto.csv')}")
    ...
    plt.tight_layout()
    out = os.path.join(current_dir, 'results_stroke_all', 'ablation_pareto.png')
    plt.savefig(out, dpi=300, bbox_inches='tight')
    plt.close()
...
def plot_pareto_front_rmse(df):
    ...
    results_dir = os.path.join(current_dir, 'results_stroke_all')
    ...
    ax.set_title('Stroke – Pareto Front: Complexity vs (1-F1)', fontsize=14, fontweight='bold')
