...
def main():
    ...
    output_root = os.path.join(current_dir, './results_bodyfat_deep')

    print("\n" + "="*70)
    print("BODYFAT DEEP ANALYSIS")
    print("="*70)
    ...
    if os.path.exists(os.path.join(output_root, 'relationships.csv')):
        print(f"Skipping bodyfat analysis as results already exist in {output_root}")
    else:
        run_deep_analysis(X, y, best_models, output_root, name='BodyFat', n_iterations=500, n_layers=3)
